...
def push_prototypes(dataloader,  # pytorch dataloader (must be unnormalized in [0,1])
    ...
    for push_iter, (search_batch_input, search_y, img_name) in enumerate(dataloader):
        ...
        protoL_input_ = np.copy(protoL_input_torch.detach().cpu().numpy())  # backbone features   # [b, 128, 16, 16]
        proto_dist_ = np.copy(proto_dist_torch.detach().cpu().numpy())      # distance maps  # [b, 750, 16, 16]
        ...
        for j in range(n_prototypes):
            ...
            for img_index_in_batch in class_to_img_index_dict[target_class]:
                proto_dist_j = proto_dist_[img_index_in_batch, j, :, :]
    ...
    for j in range(n_prototypes):
        ...
        for push_idx in sorted_idx:
            ...
            # rescaled_act_img_j = np.power(rescaled_act_img_j, 1.30)
            ...
            # draw prototype contour in the source training image
...
def imsave_with_bbox(fname, img_rgb, bbox_height_start, bbox_height_end, bbox_width_start, bbox_width_end, highest_index, color=(0, 255, 255)):
    ...
    plt.imsave(fname, img_rgb_float, vmin=0.0, vmax=1.0, pil_kwargs={'quality': 99})
